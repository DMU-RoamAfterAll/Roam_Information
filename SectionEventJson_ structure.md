...
- **flagCheck** : 선택지를 내보내기 위해 플래그를 확인하는 노드, `<"플래그명", boolean>` 형식으로 작성 (리스트 형식으로 복수 체크 가능)
- **prob** : 본문을 내보내기 위해 분기 확률을 계산하는 노드, `<"노드 키값", 확률>` 형식으로 작성 (리스트 형식으로 복수 체크 가능)
- **reset** : 유저 데이터를 초기화하는 노드, `"reset"` 형식으로 작성
