...
### Text
텍스트 본문을 출력하는 노드, `Text1`, `Text2` ... 혹은 `Result1`, `Result2`, `PostBattle_W`, `PostBattle_L` ... 등으로 작성
...
- **menuOption** : 선택지의 내용을 설정하는 노드, 리스트 형식으로 각 선택지를 구분  
  - **id** : 선택지를 구분하기 위한 키값  
  - **label** : 선택지 내용  
  - **next** : 다음 출력을 위한 노드 키값, `Result`의 형태로 작성, 본문이 출력된 후 해당 노드로 이동  
  - **action** : 키값을 주어 다양한 기능을 제어 가능
...
- **BattleWin** : 전투 승리 시 다음 출력을 위한 노드 키값, `PostBattle_W`의 형태로 작성, 본문이 출력된 후 해당 노드로 이동
- **BattleLose** : 전투 패배 시 다음 출력을 위한 노드 키값, `PostBattle_L`의 형태로 작성, 본문이 출력된 후 해당 노드로 이동
